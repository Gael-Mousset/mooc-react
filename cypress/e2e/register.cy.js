describe("Inscription ", () => {
  it("deverait afficher une erreur si le nom est trop court", () => {
    cy.visit("/register");
    cy.get("#name").type("T");
    cy.get("button[type=submit]").click();
    cy.get("div.alert.alert-warning")
      .first()
      .should("have.text", "Votre nom doit comporter au moins 2 caractères");
    cy.log();
  ...
  it("devrait afficher une erreur si email invalide", () => {
    ...
    cy.get("div.alert.alert-warning")
      .first()
      .should("have.text", "Entrez un email valide");
    cy.url().should("eq", Cypress.config().baseUrl + "register");
  });
